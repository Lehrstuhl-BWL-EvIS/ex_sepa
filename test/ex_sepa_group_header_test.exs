defmodule ExSepaGroupHeaderTest do
  ...
  describe "ExSepa.GroupHeader new" do
    ...
    test "error: key missing initiating_party_name" do
      assert ExSepa.GroupHeader.new(%{
        ...
        "initiating_party" => "Gymnastics & sports club"
               ...
               {:error,
                "key initiating_party_name is missing"}
    end
    ...
    test "error: initiating_party_name ampersand" do
      assert ExSepa.GroupHeader.new(%{
        "msg_id" => "Msg-ID-000100",
        "initiating_party_name" => "Gymnastics & sports club"
      }) ==
               {:error,
                "initiating_party_name: These characters are not part of the pattern test: &"}
    end

    test "error: initiating_party_name german Fußbalclub" do
      assert ExSepa.GroupHeader.new(%{
        "msg_id" => "Msg-ID-000100",
        "initiating_party_name" => "Fußbalclub"
      }) ==
               {:error,
                "initiating_party_name: These characters are not part of the pattern test: ß"}
    end
    ...
    test "latin character set 2" do
      assert ExSepa.GroupHeader.new(%{
        ...
        "initiating_party_name" => "René Rast"
               ...
               {:error, "msg_id: These characters are not part of the pattern test: §"}
    end
  end
end
